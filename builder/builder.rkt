...
;; Racket's file and directory copy routines have some failure modes that don't make the kind of copy I'd like
;; to do (copy recursively with directories, and copy over if they exist) particularly easy. This little
;; wrapper just does what I want it to, no questions asked.
(define (ez-move path dst)
  ;; I want the last value of the path. I can get this with 'split-path', but it returns 3 values using "values",
  ;; meaning it's not in a list. To access that middle value, I have to use "call-with-values".
